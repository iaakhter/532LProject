...
__all__ = ['ResNet', 'simplenet', 'resnet',
...
class SimpleNet(nn.Module):
    def __init__(self, nb_classes, channel):
        ...
        self.layer2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2))
        self.fc = nn.Linear(6144, nb_classes)
...
class ResNet_custom(nn.Module):
    ...
    def __init__(self, block, layers, nb_classes=101, channel=20):
        ...
        self.fc_custom = nn.Linear(128 * block.expansion, nb_classes)
        print block.expansion
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
...
def resnet(pretrained=False, channel= 3, num_classes=4, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_custom(BasicBlock, [2, 2], nb_classes=num_classes, channel=channel, **kwargs)
    if pretrained:
       pretrain_dict = model_zoo.load_url(model_urls['resnet'])                  # modify pretrain code
       model_dict = model.state_dict()
       model_dict=weight_transform(model_dict, pretrain_dict, channel)
       model.load_state_dict(model_dict)
    return model

def simplenet(pretrained=False, channel= 3, num_classes=4, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SimpleNet(nb_classes=num_classes, channel=channel, **kwargs)
    return model



def resnet18(pretrained=False, channel= 20, num_classes=4, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
